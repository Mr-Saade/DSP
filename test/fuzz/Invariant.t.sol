...
contract StablecoinEngineInvariantTest is StdInvariant, Test {
    ...
    function setUp() public {
        ...
        handler = new StablecoinEngineHandler(
            stablecoinEngine,
            stablecoin,
            weth,
            wbtc
    ...
    function invariant_TotalCirculatingStablecoinsMustBeLessThanTotalCollateralValue()
        public
        view
    {
        uint256 totalWethCollateral = ERC20Mock(weth).balanceOf(
            address(stablecoinEngine)
        );
        uint256 totalWbtcCollateral = ERC20Mock(wbtc).balanceOf(
            address(stablecoinEngine)
        );
        uint256 totalWethCollateralValue = stablecoinEngine.getCollateralValue(
            totalWethCollateral,
            stablecoinEngine.s_wethPriceFeed()
        );
        uint256 totalWbtcCollateralValue = stablecoinEngine.getCollateralValue(
            totalWbtcCollateral,
        ...
        console.log("totalStablecoinSupply", totalStablecoinSupply);
        console.log("totalCollateralValue", totalCollateralValue);
        assert(totalStablecoinSupply <= totalCollateralValue);
        assertEq(
            handler.totalCollateralDeposited(),
            totalWethCollateral + totalWbtcCollateral
        );
    }
}
